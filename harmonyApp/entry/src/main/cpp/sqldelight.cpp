...
libkn_kref_app_cash_sqldelight_db_SqlDriver createTestDriver() {
    ...
    std::string dbPath =
        g_dbPath.empty() ? "/data/storage/el2/base/hips/entry/files/database/my_database.db" : g_dbPath;
    LOGI("数据库Database path: %s", dbPath.c_str());
    LOGI("开始创建数据库驱动");
    LOGI("数据库路径: %s", dbPath.c_str());
    LOGI("数据库路径长度: %zu", dbPath.length());
    LOGI("数据库路径是否为空: %s", dbPath.empty() ? "是" : "否");
    // 确保数据库目录存在
    ...
    if (stat(dbPath.c_str(), &st) == 0) {
        ...
        LOGI("Database file does not exist yet, will be created by SQLDelight\n");
    }

    // 创建 SQLite 驱动
    auto kt = libkn_symbols();
    LOGI("Loading Kotlin symbols\n");
